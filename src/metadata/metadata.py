...
class Metadata():
    ...
    def add_file(self, file_name, file_length):
        ...
        self.files[file_name] = self.manager.dict({'cloud': cloud_name, 'length': file_length, 'accesses': 0})
    ...
    def cloud_outliers(self, cloud_name):
        ...
        cloud_files_info = [(file_name, file['accesses']) for file_name, file in self.files.items() if file['cloud'] == cloud_name]
        ...
        for cloud_id in range(0, len(self.clouds)):
            ...
            if cloud_id != 0 and lower_outliers != []:
                for file_name in lower_outliers:
                    clouds_migration_data.append((file_name, cloud_id, cloud_id - 1))
